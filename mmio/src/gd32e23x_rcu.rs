...
/* RCU definitions */
//pub const RCU: u32 = RCU_BASE;
pub const RCU: u32 = 0x4000_1000;
...
/* constants definitions */
/* define the peripheral clock enable bit position and its register index offset */
macro_rules! RCU_REGIDX_BIT  { ($regidx:expr,$bitpos:expr) => ((($regidx as u32)<<6) | $bitpos) }
macro_rules! RCU_REG_VAL     { ($periph:expr) => ((RCU + (($periph as u32)>>6)) as u32) }
...
#[repr(C)]
struct Rcu {
    pub rcu_ctl0: [RW<u32>; 32],
    pub rcu_cfg0: [RW<u32>; 32],
    pub rcu_int: [RW<u32>; 32],
    pub rcu_apb2rst: [RW<u32>; 32],
    pub rcu_apb1rst: [RW<u32>; 32],
    pub rcu_ahben: [RW<u32>; 32],
    pub rcu_apb2en: [RW<u32>; 32],
    pub rcu_apb1en: [RW<u32>; 32],
    pub rcu_bdctl: [RW<u32>; 32],
    pub rcu_rstsck: [RW<u32>; 32],
    pub rcu_ahbrst: [RW<u32>; 32],
    pub rcu_cfg2: [RW<u32>; 32],
    pub rcu_ctl1: [RW<u32>; 32],
}
...
enum RcuPeriphEnum {
    /* AHB peripherals */
    RcuDMA     = RCU_REGIDX_BIT!(RegIdx::IdxAHBEN, 0),                  // DMA clock
    RcuCRC     = RCU_REGIDX_BIT!(RegIdx::IdxAHBEN, 6),                  // CRC clock
    RcuGPIOA   = RCU_REGIDX_BIT!(RegIdx::IdxAHBEN, 17),                 // GPIOA clock
    RcuGPIOB   = RCU_REGIDX_BIT!(RegIdx::IdxAHBEN, 18),                 // GPIOB clock
    RcuGPIOC   = RCU_REGIDX_BIT!(RegIdx::IdxAHBEN, 19),                 // GPIOC clock
    RcuGPIOF   = RCU_REGIDX_BIT!(RegIdx::IdxAHBEN, 22),                 // GPIOF clock
    
    /* APB2 peripherals */
    RcuCFGCMP  = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 0),                 // CFGCMP clock
    RcuADC     = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 9),                 // ADC clock
    RcuTIMER0  = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 11),                // TIMER0 clock
    RcuSPI0    = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 12),                // SPI0 clock
    RcuUSART0  = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 14),                // USART0 clock
    RcuTIMER14 = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 16),                // TIMER14 clock
    RcuTIMER15 = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 17),                // TIMER15 clock
    RcuTIMER16 = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 18),                // TIMER16 clock
    RcuDBGMCU  = RCU_REGIDX_BIT!(RegIdx::IdxAPB2EN, 22),                // DBGMCU clock
    
    /* APB1 peripherals */
    RcuTIMER2  = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 1),                 // TIMER2 clock
    RcuTIMER5  = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 4),                 // TIMER5 clock
    RcuTIMER13 = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 8),                 // TIMER13 clock
    RcuWWDGT   = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 11),                // WWDGT clock
    RcuSPI1    = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 14),                // SPI1 clock
    RcuUSART1  = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 17),                // USART1 clock
    RcuI2C0    = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 21),                // I2C0 clock
    RcuI2C1    = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 22),                // I2C1 clock
    RcuPMU     = RCU_REGIDX_BIT!(RegIdx::IdxAPB1EN, 28),                // PMU clock
    
    /* Backup domain control(BDCTL) */
    RcuRTC     = RCU_REGIDX_BIT!(RegIdx::IdxBDCTL, 15)                  // RTC clock
}
...
/**!
    ...
    \param[in]  periph: RCU peripherals, refer to rcu_periph_enum
      ...
      \arg        RcuGPIOx (x=A,B,C,F): GPIO ports clock
      \arg        RcuDMA: DMA clock
      \arg        RcuCRC: CRC clock
      \arg        RcuCFGCMP: CFGCMP clock
      \arg        RcuADC: ADC clock
      \arg        RcuTIMERx (x=0,2,5,13,14,15,16): TIMER clock
      \arg        RcuSPIx (x=0,1): SPI clock
      \arg        RcuUSARTx (x=0,1): USART clock
      \arg        RcuWWDGT: WWDGT clock
      \arg        RcuI2Cx (x=0,1): I2C clock
      \arg        RcuPMU: PMU clock
      \arg        RcuRTC: RTC clock
      \arg        RcuDBGMCU: DBGMCU clock
    \param[out] none
    \retval     none
**/
...
//    RCU_REG_VAL(periph) |= BIT(RCU_BIT_POS(periph));
    ...
    //let rcu = RCU as *const Rcu;
    let rcu = RCU as *const [RW<u32>; 13];
    let mut val: u32;

    unsafe { 
        val = (*rcu)[periph as usize].read();
        val |= BIT!(RCU_BIT_POS!(periph));
        (*rcu)[periph as usize].write(val);
    }
}
